 ...
 * Probes existing server on port 3001 or starts one in DEMO_MODE
 */
      ...
      res.on('end', () => {
        try {
          ...
          resolve({
            ...
            demoMode: json && json.demoMode === true
          });
        } catch (e) {
          resolve({
            alive: res.statusCode >= 200 && res.statusCode < 500,
...
async function ensureServerRunning() {
  ...
  if (status.alive) {
    ...
    console.warn(`\n⚠️  [SAFETY WARNING] Existing server at ${config.BASE_URL} is NOT running in DEMO_MODE.`);
    console.warn(`    Spawning isolated test server on port ${isolatedPort} with DEMO_MODE=true...\n`);
...
module.exports = {
  checkServerStatus,
  checkServerAlive: async (baseUrl) => (await checkServerStatus(baseUrl)).alive,
  ensureServerRunning,
  stopServer
};
